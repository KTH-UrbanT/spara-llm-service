...
logging.basicConfig(
    ...
    # handlers=[
    #     logging.FileHandler("conversational_agent.log"), # Log to a file (optional, uncomment to enable)
    #     logging.StreamHandler(sys.stdout)       # Log to console
    # ]
)
logger = logging.getLogger(__name__)

class ConversationalAgent(BaseAgent): # Inherit from BaseAgent
    """
    A class to interact with Azure OpenAI's conversational models.
    Handles prompt loading, API client initialization, and chat completions.
    Inherits from BaseAgent.
    """

    # Default path for the conversational prompt file
    DEFAULT_PROMPT_RELATIVE_PATH = os.path.join("..", "prompts", "conversational_prompt.txt")

    def __init__(self, prompt_path: Optional[str] = None):
        """
        Initializes the ConversationalAgent with Azure OpenAI configurations
        and loads the conversational prompt.

        Args:
            prompt_path (Optional[str]): The absolute path to the prompt file.
                                         If None, a default relative path is used.
        Raises:
            ValueError: If required environment variables are not set or prompt file is not found.
        """
        super().__init__() # Call the constructor of the base class (BaseAgent)
        logger.info("Initializing ConversationalAgent.")

        try:
            self.prompt_template = self._load_prompt(prompt_path)
            logger.info(f"ConversationalAgent prompt template loaded from: {prompt_path or os.path.join(os.path.dirname(__file__), self.DEFAULT_PROMPT_RELATIVE_PATH)}")
        except (ValueError, IOError) as e:
            logger.critical(f"Failed to load prompt for ConversationalAgent: {e}")
    ...
    def _load_prompt(self, path: Optional[str]) -> str:
        """
        Loads the conversational prompt from a specified file path.
    ...
    def _initialize_openai_client(self):
        ...
        required_env_vars = {
            ...
            "CONVERSATIONAL_MODEL_DEPLOYMENT_NAME": "Deployment name for the conversational model",
            "OPENAI_API_KEY": "OpenAI API key",
            "CONVERSATIONAL_MODEL_API_VERSION": "API version for the conversational model"
        }
        ...
        self.deployment = os.getenv("CONVERSATIONAL_MODEL_DEPLOYMENT_NAME")
        subscription_key = os.getenv("OPENAI_API_KEY")
        api_version = os.getenv("CONVERSATIONAL_MODEL_API_VERSION")
        logger.info(f"Conversational Model is {self.deployment} and the api version is {api_version}. ")
        try:
    ...
    def handle_conversational_input(self, last_message: str, message_list: List[Dict[str, Union[str, int]]]) -> Optional[str]:
        start_time = time.time()
        response_language = response_language_for_message(last_message, messages=message_list)
        """
        Generates a conversational response using the Azure OpenAI model.

        Args:
            last_message (str): The content of the most recent message from the user.
            message_list (List[Dict[str, Union[str, int]]]): A list of previous messages in the conversation,
                                                 each with at least 'role' and 'content' keys,
                                                 and potentially other keys like 'timestamp', 'added_to_database', etc.

        Returns:
            Optional[str]: The content of the AI's response, or None if an error occurs.
        """
        logger.info(f"Received conversational input. Last message: '{last_message[:70]}...'")
