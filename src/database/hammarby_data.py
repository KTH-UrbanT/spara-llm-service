...
load_dotenv()  # pulls server, database, username, password, port from .env when not already in os.environ. Do NOT use override=True — it silently clobbers EVALUATOR_MODE, which scripts/run_arm_closed_loop.py sets per-arm before import; letting .env win would run every arm under the ambient mode and invalidate the comparison (plan-eil-v3.md Phase 1 appendix).
