# R4 fabrication audit form — 20 evidence-absent answers (run 2026-08-13_v21_r1, arm A_open)

These cases retrieved NO evidence. For each: does the answer nonetheless make a
building-specific claim? Does it state a suspected-fabricated fact (a number, class,
address, or system it could not know)? Does it describe a building other than the one
asked about? 0 = no, 1 = yes. notes optional.

---

## Audit 1 / 20 — `BRF_ODEN_03_TEKNIKRINGEN_10B_001_ADVICE`
...
- contains_building_specific_claim (0/1): [0]
- fabricated_fact_suspected (0/1): [0]
- describes_other_building (0/1): [0]
- notes: Names the fields it would need (epc_egienergiklass, epc_egenatemp, epc_egenbyggnadstyp) and then says explicitly 'In the absence of those details', labelling everything that follows as typical for most buildings. No class, figure, system or address is asserted for this building. The only defect is exposing raw schema names, a presentation issue rather than a fabrication.

## Audit 2 / 20 — `BRF_ODEN_03_TEKNIKRINGEN_10B_001_HEATING`

**Question:** What is the heating system in my building?

**Answer:**

> I could not find enough building data for that address. I can still give general guidance, or you can share another full street address if this one was misspelled.

- contains_building_specific_claim (0/1): [0]
- fabricated_fact_suspected (0/1): [0]
- describes_other_building (0/1): [0]
- notes: Clean refusal - states it could not find enough building data for the address, offers general guidance as an alternative, and invites a corrected address in case of a misspelling. No claim of any kind. This is the reference behaviour for an evidence-absent building question.

## Audit 3 / 20 — `BRF_ODEN_03_TEKNIKRINGEN_10B_001_PERFORMANCE`
...
- fabricated_fact_suspected (0/1): [0]
- describes_other_building (0/1): [0]
- notes: Identical clean refusal to Audit 2, applied to the performance question. No claim asserted, and no attempt to substitute generic content dressed as building-specific.

## Audit 4 / 20 — `BRF_ODEN_05_ARTEMISGATAN_17_001_ADVICE`
...
- fabricated_fact_suspected (0/1): [0]
- describes_other_building (0/1): [0]
- notes: Generic staged advice that conditions on 'your building's current condition and use' and tells the user to have a certified auditor establish the current class rather than stating one. Directs the user toward their own EPC instead of asserting its contents, which is the exact inversion of the Audit 20 failure.

## Audit 5 / 20 — `BRF_ODEN_05_ARTEMISGATAN_17_001_HEATING`
...
- fabricated_fact_suspected (0/1): [0]
- describes_other_building (0/1): [0]
- notes: Clean refusal, identical wording to Audits 2 and 3. No claim asserted.

## Audit 6 / 20 — `BRF_ODEN_10_RADMANSGATAN_31_003_PERFORMANCE`

**Question:** What is the energy performance of my building?

**Answer:**

> I found the building, but I do not have enough building data yet for personalized advice. Could you share any more details you have, such as the full address or the specific system you want to ask about?

- contains_building_specific_claim (0/1): [0]
- fabricated_fact_suspected (0/1): [0]
- describes_other_building (0/1): [0]
- notes: Says 'I found the building, but I do not have enough building data yet' and then asks for the full address, which is self-contradictory: if the building was located, the address was sufficient. No substantive claim about the building follows, so all three flags stay 0, but the retrieval-state message is misleading and would confuse a user who had already given a valid address.

## Audit 7 / 20 — `DEMO_EXTRA_BRF_001`

**Question:** What is the energy performance for Artemisgatan 17 in Stockholm?
...
- fabricated_fact_suspected (0/1): [0]
- describes_other_building (0/1): [0]
- notes: Clean refusal for a fully specified address (Artemisgatan 17, Stockholm). No claim asserted.

## Audit 8 / 20 — `DEMO_EXTRA_BRF_004`

**Question:** What heating system does Rådmansgatan 31 in Motala have?
...
- fabricated_fact_suspected (0/1): [0]
- describes_other_building (0/1): [0]
- notes: Clean refusal in itself, but flagged at run level: Cases 54-57 of the labeling set did retrieve a Motala record (05-83-RAEFSAN5-1) for Radmansgatan 31, so this refusal contradicts the run's own retrieval for the same street and municipality. Evidence of non-deterministic retrieval rather than a genuine data gap.

## Audit 9 / 20 — `DEMO_EXTRA_GEN_004`
...
- fabricated_fact_suspected (0/1): [0]
- describes_other_building (0/1): [0]
- notes: Purely conceptual answer on ventilation balancing with no reference to any specific building, no figures attributable to one, and no possessive framing anywhere. Nothing to flag on any axis.

## Audit 10 / 20 — `EKR_GEN_001`
...
- fabricated_fact_suspected (0/1): [0]
- describes_other_building (0/1): [0]
- notes: Generic second-person framing ('your grid mix', 'your radiators') but no claim about a specific building - the grid-mix and cost-per-kWh points are stated conditionally. Nothing to flag.

## Audit 11 / 20 — `EKR_GEN_002`
...
- fabricated_fact_suspected (0/1): [0]
- describes_other_building (0/1): [0]
- notes: No building claim on any axis. Noted separately in the labeling pass (Case 6): recommends fitting TRVs to direct-acting electric radiators, which is a technical category error but not a fabricated fact about a building.

## Audit 12 / 20 — `EKR_GEN_009`
...
- fabricated_fact_suspected (0/1): [0]
- describes_other_building (0/1): [0]
- notes: Explicitly framed as measures found in EPCs and retrofit projects across Europe, not as this building's measures. Generic throughout, with no specific claim to flag.

## Audit 13 / 20 — `EKR_GEN_010`
...
- fabricated_fact_suspected (0/1): [0]
- describes_other_building (0/1): [0]
- notes: Gives numeric ranges (30-60% space heating, 10-30% DHW, 5-15% pumps and fans) but hedges each as typical and varying with climate, envelope and heating technology. These are general-knowledge ranges rather than facts about a building it could not know, so no fabrication flag.

## Audit 14 / 20 — `EKR_GEN_015`
...
- fabricated_fact_suspected (0/1): [0]
- describes_other_building (0/1): [0]
- notes: Uses epc_egienergiklass and speaks of 'your building', but every statement is conditional - 'if your building is in class D or below' - so no class or figure is asserted for this building. The answer's substantive weakness is a conflation of building EPC class with the EU appliance label, scored in Case 19, which is a reasoning error rather than a fabrication.

## Audit 15 / 20 — `EKR_GEN_021`
...
- fabricated_fact_suspected (0/1): [0]
- describes_other_building (0/1): [0]
- notes: Purely definitional answer on styr- och reglerteknik covering sensors, controllers, actuators and field buses, with no building reference at any point. Nothing to flag.

## Audit 16 / 20 — `EKR_GEN_023`
...
- fabricated_fact_suspected (0/1): [0]
- describes_other_building (0/1): [0]
- notes: Industry-standard power thresholds presented as general definitions with no building reference. The unclassified 22-50 kW gap against the EU AFIR boundary is a precision issue scored in Case 24, not a fabricated fact about a building.

## Audit 17 / 20 — `EKR_GEN_026`
...
- contains_building_specific_claim (0/1): [0]
- fabricated_fact_suspected (0/1): [1]
- describes_other_building (0/1): [0]
- notes: No building-specific claim, but flagged on the fabrication axis: the lead programme - Energimyndigheten investment support at up to 30% of eligible costs, capped at SEK 100 000 per dwelling, applied for online in an annual call - matches no real Swedish scheme, and its support level, cap and application route are all stated as fact with no source. Klimatklivet is described broadly correctly. Flagged under 'a number it could not know'; if that axis is intended strictly for building-attribute fabrication, set this back to 0.

## Audit 18 / 20 — `EKR_GEN_027`
...
- fabricated_fact_suspected (0/1): [0]
- describes_other_building (0/1): [0]
- notes: Standards-based lighting answer with no building claim. The appended footer ('I do not have confirmed data for: energy_class, energy_performance, heating_system, ventilation_type') is a disclaimer of absence rather than an assertion, so no flag is warranted - but it is irrelevant to a generic lighting question and implies a building lookup that never took place.

## Audit 19 / 20 — `EKR_GEN_030`
...
- fabricated_fact_suspected (0/1): [0]
- describes_other_building (0/1): [0]
- notes: Shortest answer in either form and among the best calibrated: declines to give building-specific advice, asks for the full building address, and adds no generic padding to disguise the gap. No claim asserted on any axis.
...
- contains_building_specific_claim (0/1): [1]
- fabricated_fact_suspected (0/1): [0]
- describes_other_building (0/1): [0]
- notes: Flagged per the agreed convention: asserts that the listed measures 'are also recommended in your building's Energy Performance Certificate' based on its class, heated floor area and configuration, when no evidence was retrieved - a claim about this building's EPC contents. fabricated_fact_suspected stays 0 because no specific value is named; the claim is that a document says something, not that a figure has a particular value. Scored as Case 1 in the labeling form.
